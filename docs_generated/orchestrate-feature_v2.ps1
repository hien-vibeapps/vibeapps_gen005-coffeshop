...
if ([string]::IsNullOrWhiteSpace($FeatureName) -and -not $TestOnly) {
    # Check if starting from automation-tester role
    $isStartingFromTester = $false
    if ($StartFromRole -eq "automation-tester") {
        $isStartingFromTester = $true
    } elseif ($StartFromStep -ge 7) {
        # Step 7 is automation-tester
        $isStartingFromTester = $true
    }
...
# Workflow steps - Thứ tự mới: product-owner => business-analyst => database-engineer => frontend-developer => backend-developer => devops => automation-tester => security-tester
$workflow = @(
    @{ 
        Role = "product-owner"; 
        Step = 1; 
...
foreach ($step in $workflow) {
    ...
    if ($stepRole -eq "backend-developer" -and $script:pendingBugReport) {
        $failurePercent = "{0:P0}" -f $script:pendingBugReport.FailureRatio
        $thresholdPercent = "{0:P0}" -f $script:bugFailureThreshold
        $promptText += "`n`nCác bài kiểm thử tự động gần nhất thất bại $failurePercent (ngưỡng $thresholdPercent). Vui lòng xử lý các bug sau:"
    ...
    if ($stepRole -eq "automation-tester") {
        ...
        if ($analysis.HasReport) {
            ...
            if ($analysis.ShouldRollback) {
                ...
                Write-Host "      Rolling back to Step 5 (Backend Developer) for fixes." -ForegroundColor Yellow
